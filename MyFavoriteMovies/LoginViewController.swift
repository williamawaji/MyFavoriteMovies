...
class LoginViewController: UIViewController {
    ...
    @IBAction func loginPressed(_ sender: AnyObject) {
        ...
        if usernameTextField.text!.isEmpty || passwordTextField.text!.isEmpty {
            ...
            // MARK: Store username and password in struct Constants
    ...
    private func getRequestToken() {
        ...
        let task = appDelegate.sharedSession.dataTask(with: request) { (data, response, error) in
            ...
            /* GUARD: There was any data returned? */
            ...
            /* GUARD: Check success on parsed response */
            ...
            /* GUARD: Is the "request_token" key parsed? */
            guard let requestToken = parsedResult[Constants.TMDBResponseKeys.RequestToken] as? String else {
                displayError("Cannot find the key '\(Constants.TMDBResponseKeys.RequestToken)' in \(parsedResult)")
                return
            }
    ...
    private func loginWithToken(_ requestToken: String) {
        ...
        let task = appDelegate.sharedSession.dataTask(with: request) { (data, response, error) in
            ...
            func displayError(_ error: String) {
                ...
                performUIUpdatesOnMain {
                    self.setUIEnabled(true)
                    self.debugTextLabel.text = "Login With Token Failed."
                }
            }
    ...
    private func getSessionID(_ requestToken: String) {
        ...
        let task = appDelegate.sharedSession.dataTask(with: request) { (data, response, error) in
            ...
            func displayError(_ error: String) {
                ...
                performUIUpdatesOnMain {
                    self.setUIEnabled(true)
                    self.debugTextLabel.text = "Failed to get sessionID"
                }
            }
            ...
            /* GUARD: check reponse status code? */
            guard let statusCode = (response as? HTTPURLResponse)?.statusCode, statusCode >= 200 && statusCode <= 299 else {
                displayError("Your request returned a status code other than 2xx.")
                return
            }
            
            /* GUARD: check data content */
            guard let data = data else {
                displayError("There is no data returned by the request.")
                return
            }
    ...
    private func getUserID(_ sessionID: String) {
        ...
        let task = appDelegate.sharedSession.dataTask(with: request) { (data, response, error) in
            ...
            func displayError(_ error: String) {
                ...
                performUIUpdatesOnMain {
                    self.setUIEnabled(true)
                    self.debugTextLabel.text = "Failed to get UserID"
                }
            }
